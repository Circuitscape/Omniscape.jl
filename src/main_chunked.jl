...
**`chunks`**: Define if and how the landscape should be chunked into tiles
which will be solved in parallel if running `addprocs`
...
function run_omniscape(
    ...
    if nthreads() > 1
        @info("Starting up Omniscape with $(nthreads()) workers and $(precision_name) precision")
    else
        @info("Starting up Omniscape in serial with $(precision_name) precision")
    end
    ...
    cum_currmap = fill(convert(precision, 0.),
                        int_arguments["nrows"],
                        int_arguments["ncols"])
    cum_currmap = Dagger.tochunk(cum_currmap, Dagger.OSProc(), Dagger.ProcessScope())

    if os_flags.calc_flow_potential || os_flags.calc_normalized_current
        fp_cum_currmap = fill(convert(precision, 0.),
                                int_arguments["nrows"],
                                int_arguments["ncols"])
        fp_cum_currmap = Dagger.tochunk(fp_cum_currmap, Dagger.OSProc(), Dagger.ProcessScope())
    else
        # Hacky fix -- a later function needs fp_cum_currmap to be an array
        fp_cum_currmap = Array{precision, 2}(undef, 1, 1)
    end
    ...
    for i in 0:(n_batches - 1)
        ...
        @sync for j in start_ind:end_ind
            target = Target(Int64(targets[j, 1]), Int64(targets[j, 2]), float(targets[j, 3]))
            try 
                @async Dagger.@spawn solve_target!( # change this to Threads.@spawn and it works (also need to comment out lines 212 and 218)
                    cum_currmap,
                    fp_cum_currmap,
                    target,
                    int_arguments,
        ...
        cum_currmap = collect(cum_currmap)
        fp_cum_currmap = collect(fp_cum_currmap)
    end
